...
$id_detail = $_GET['id'];
$query = mysqli_query($con, "SELECT * FROM detail_peminjaman WHERE detail_peminjaman.id_detail='$id_detail'") or die (mysqli_error($con));
...
<head>
  ...
  <title>Inventaris | Form Edit Detail Peminjaman</title>
        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">Edit Detail Peminjaman</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
        ...
        <div class="row">
          ...
          <div class="col-md-6">
            ...
            <div class="card-header">
                <h3 class="card-title"><i class="ion-compose mr-2"></i>Edit Detail Peminjaman</h3>
              </div>
              <!-- /.card-header -->
              <!-- form start -->
              <form method="post" action="?id=<?=$_GET['id'];?>">
                <div class="card-body">
                  <input type="hidden" class="form-control" name="id_peminjaman" value="<?=$data['id_peminjaman'];?>">
                  <input type="hidden" class="form-control" name="id_detail" value="<?=$data['id_detail'];?>">
                  <div class="form-group">
                    <label for="kd_barang">Kode Barang</label>
                    <select name="kd_barang" class="form-control" onchange='changeValue(this.value)' id="kd_barang">
                      <option value="">-PiLIH-</option>
                      <?php
                      $query = mysqli_query($con,"SELECT * FROM daftar_barang order by kd_barang esc");
                      $result = mysqli_query($con,"SELECT * FROM daftar_barang");
                      $data1 = "var nama_barang = new Array();\n;";
                      $data2 = "var id_barang = new Array();\n;";
                      while ($row = mysqli_fetch_array($result)) {
                        echo '<option name="kd_barang" value="'.$row['kd_barang'] . '">' . $row['kd_barang'] . $row['nama_barang'] . '</option>';
                        $data1 .= "nama_barang['" . $row['kd_barang'] . "'] = {nama_barang:'" . addslashes($row['nama_barang'])."'};\n";  
                        $data2 .= "id_barang['" . $row['kd_barang'] . "'] = {id_barang:'" . addslashes($row['id_barang'])."'};\n";  
                        }  
                      ?>
                    </select>
                  </div>
                  ...
                  <div class="form-group">
                    ...
                    <div class="input-group">
                        <input type="number" class="form-control" name="jumlah_peminjaman" placeholder="Masukan Jumlah" required>
                    </div>
                  </div>
                </div>
                <!-- /.card-body -->
                ...
                <div class="card-footer">
                  <button type="submit" class="btn btn-primary" name="edit_detail">Update</button>
                </div>
